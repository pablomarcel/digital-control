...
python cli.py solve \
  ...
  --D "1,0,0,0" \
  --layout ogata \
  --export_json out/solve_7_1.json \
  --export_csv  out/solve_7_1.csv
```
...
python cli.py polydesign \
  ...
  --backend mpl \
  --kmax 60 \
  --save out/pd_cfg2_mpl \
  --export_json out/pd_cfg2_mpl.json \
  --export_csv  out/pd_cfg2_mpl.csv
```
...
python cli.py polydesign \
  ...
  --backend none \
  --kmax 1 \
  --export_json out/pd_no_plot.json
```
...
python cli.py rst \
  ...
  --backend mpl \
  --kmax 40 \
  --save out/rst_cfg2_mpl \
  --export_json out/rst_cfg2_mpl.json
```
...
### 4.1 Plotly output
```bash
python cli.py modelmatch \
  --A "1,-1.3679,0.3679" \
  --B "0.3679,0.2642" \
  --Gmodel_num "0.62,-0.3" \
  --Gmodel_den "1,-1.2,0.52" \
  --H1 "1,0.5" \
  --F "1,0" \
  --layout ogata \
  --backend plotly \
  --kmax 40 \
  --save out/ex7_4/ex7_4_plotly
```
