...
python cli.py \
  --z \
  --expr "exp(-a*T*k)*sin(w*T*k)" \
  --subs "T=1,a=0.3,w=1.2" \
  --pretty
```
...
python cli.py \
  --iz \
  --X "(1 - exp(-T))*z**-1/((1 - z**-1)*(1 - exp(-T)*z**-1))" \
  --subs "T=1" \
  --N 0
```
...
python cli.py \
  --iz \
  --X "1 + 2*z**-1 + 3*z**-2 + 4*z**-3" \
  --N 6 \
  --export_json x_e212.json \
  --pretty
```
...
python -m z_transform.zTransformTool.cli sphinx-skel z_transform/zTransformTool/docs
