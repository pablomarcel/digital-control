...
```bash
cd digitalControl/polePlacement/poleTool
```
...
```bash
python cli.py \
  --A "0 1; -0.16 -1" --B "0; 1" --C "1 0" \
  --poles "0.5+0.5j,0.5-0.5j" \
  --plot mpl --style dots --pretty \
  --save_json --save_csv --name ex6_6_ack_mpl
```
...
python cli.py \
  --A "0 1; -0.16 -1" --B "0; 1" --C "1 0" \
  --poles "0.5+0.5j,0.5-0.5j" \
  --plot mpl --style stairs \
  --save_json --save_csv --name ex6_6_ack_mpl_stairs
```
...
python cli.py \
  --A "0 1; -0.16 -1" --B "0; 1" --C "1 0" \
  --poles "0.5+0.5j,0.5-0.5j" \
  --plot plotly --style connected \
  --save_json --save_csv --name ex6_6_ack_plotly
```
...
python cli.py \
  ...
  --C "1 0 0" \
  --poles "0.2,0.3,0.4" \
  --method place --plot none --pretty \
  --save_json --name mimo_place
```
...
python cli.py \
  --A "0 1; -0.16 -1" --B "0; 1" --C "1 0" \
  --deadbeat --plot mpl --style dots --pretty \
  --save_json --save_csv --name ex6_7_deadbeat
```
...
python cli.py \
  ...
  --poles "0.5+0.5j,0.5-0.5j" \
  --plot mpl --style dots --pretty \
  --save_json --save_csv --name from_json
```
...
python cli.py \
  --A "0 1; -0.16 -1" --B "0; 1" --C "1 0" \
  --poles "0.5+0.5j,0.5-0.5j" \
  --samples 120 --plot mpl --style dots \
  --name long_run
```
...
python cli.py \
  ...
  --poles "0.5+0.5j,0.5-0.5j" \
  --plot mpl --style dots \
  --name custom_out --outdir ./tmp/poletool_out
```
...
```bash
python -m polePlacement.poleTool.cli --help
```
