...
import pytest
from polePlacement.servoTool import io as iox
from polePlacement.servoTool.utils import out_path
from polePlacement.servoTool.design import servo_eqs, observer_eqs, emit_equations
...
def test_out_path_basename():
    p = out_path("foo.json")
    assert p.endswith(os.path.join("polePlacement","servoTool","out","foo.json"))
...
def test_yaml_optional_missing(monkeypatch, tmp_path):
    ...
    import polePlacement.servoTool.io as mod
