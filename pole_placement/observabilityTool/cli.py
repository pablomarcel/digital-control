...
# -*- coding: utf-8 -*-
"""
Command-line interface for :mod:`pole_placement.observabilityTool`.
...
_PACKAGE = "pole_placement.observabilityTool"
_PROJECT = "Digital Control - pole_placement.observabilityTool"
...
_OPTIONAL_MODULES = [
    ...
    "pole_placement.observabilityTool.plotting",
...
# docs -> observabilityTool -> pole_placement -> repository root
...
def _add_run_arguments(p: argparse.ArgumentParser) -> None:
    """Add observability analysis arguments to ``p``."""
    src = p.add_mutually_exclusive_group(required=True)
    src.add_argument("--json", type=str, help="Path to JSON system file. Expects keys A, C, and optionally discrete.")
    ...
    p.add_argument("--horizon", type=int, default=None, help="Observability horizon. Default is n.")
    ...
    p.add_argument("--gram", action="store_true", help="Compute infinite-horizon observability Gramian if stable.")
    p.add_argument("--finite-dt", type=int, default=None, help="Compute finite-horizon discrete-time observability Gramian over N steps.")
    p.add_argument("--finite-ct", type=float, default=None, help="Compute finite-horizon continuous-time observability Gramian over T seconds.")
    p.add_argument("--minreal", action="store_true", help="Trim unobservable modes and write *_minreal.json.")
    p.add_argument("--symbolic", action="store_true", help="Run SymPy exact-rank test on the observability matrix.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="pole_placement.observabilityTool",
        description=(
            "Ogata-grade observability toolbox for continuous-time and "
            "discrete-time systems: observability matrices, PBH dual tests, "
            "Gramians, symbolic rank, and unobservable-mode trimming."
        ),
    ...
    run = sub.add_parser(
        "run",
        help="run observability analysis",
        description=(
            "Run observability checks, PBH dual tests, Gramian analysis, "
            "symbolic rank checks, or unobservable-mode trimming."
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        allow_abbrev=False,
    )
...
def _run_observability(ns: argparse.Namespace) -> int:
    """Run the observability analysis workflow."""
    req = RunRequest(
        json_in=ns.json,
        A=ns.A,
        C=ns.C,
