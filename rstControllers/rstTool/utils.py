...
_PKG_DIR = os.path.abspath(os.path.dirname(__file__))              # .../rstControllers/rstTool
...
def pkg_out_path(name: Optional[str], default_basename: str) -> Optional[str]:
    """
    Always write under rstControllers/rstTool/out regardless of where the process is launched.
    """
...
def pkg_in_path(name: Optional[str]) -> Optional[str]:
    """
    Resolve input under rstControllers/rstTool/in, handling a few common spellings:
    - "foo.json"         -> <pkg>/in/foo.json
    - "in/foo.json"      -> <pkg>/in/foo.json
    - "rstControllers/rstTool/in/foo.json" -> <pkg>/in/foo.json
    ...
    prefix = "rstControllers/rstTool/in/"
