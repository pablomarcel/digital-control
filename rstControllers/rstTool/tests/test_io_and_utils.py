...
from rstControllers.rstTool.io import parse_coeffs, parse_complex_list, as_array, save_json_design, save_csv, load_json
from rstControllers.rstTool.utils import pkg_out_path, pkg_in_path
...
def test_parse_and_paths(tmp_path, monkeypatch):
    ...
    assert outp.endswith("rstControllers/rstTool/out/my.csv")
    inp = pkg_in_path("foo.json")
    assert inp.endswith("rstControllers/rstTool/in/foo.json")
...
def test_save_and_load_json_and_csv(tmp_path):
    ...
    # move to in/ and load
    in_dir = os.path.join("rstControllers","rstTool","in")
