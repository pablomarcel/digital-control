...
```bash
pytest rstControllers/rstTool/tests
```

Ad-hoc coverage focused on rstTool only (ignores repo addopts):
```bash
pytest rstControllers/rstTool/tests   --override-ini addopts=   --cov=rstControllers.rstTool   --cov-report=term-missing
```
...
```bash
python -m rstControllers.rstTool.cli --A "1 -0.8" --B "0.5" --d 1 --poles "0.6 0.6" --Tmode unity_dc --pretty
```
